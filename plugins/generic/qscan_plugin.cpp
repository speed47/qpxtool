...
scan_generic::scan_generic(drive_info* idev) : scan_plugin(), lba(0) {
	dev = idev;
...
int scan_generic::errc_data() {
	if (dev->media.type & DISC_CD) { return (ERRC_DATA_E22 | ERRC_DATA_UNCR); }
	return 0;
}
...
int scan_generic::check_test(unsigned int itest) {
	switch (itest) {
		case CHK_ERRC:
			if (dev->media.type & DISC_CD) return 0;
			break;
		default:
			break;
	}
