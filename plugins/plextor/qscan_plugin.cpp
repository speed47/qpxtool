...
static const int SPEEDS_ERRC_CD[] = {4 * CD_SPEED_MULT,  8 * CD_SPEED_MULT,  24 * CD_SPEED_MULT,
                                     32 * CD_SPEED_MULT, 40 * CD_SPEED_MULT, 0};

static const int SPEEDS_ERRC_DVD[] = {2 * DVD_SPEED_MULT, 5 * DVD_SPEED_MULT, 8 * DVD_SPEED_MULT, 12 * DVD_SPEED_MULT,
                                      0};
...
int scan_plextor::check_test(unsigned int itest) {
	switch (itest) {
		...
		case CHK_JB:
			...
			break;
		case CHK_FETE:
			if (dev->media.type & (DISC_CD | DISC_DVD) & ~DISC_CDROM & ~DISC_DVDROM) return 0;
			break;
		case CHK_TA:
...
int* scan_plextor::get_test_speeds(unsigned int itest) {
	switch (itest) {
		case CHK_ERRC:
			...
			break;
		case CHK_JB:
			if (dev->media.type & DISC_CD) return (int*)SPEEDS_JB_CD;
			if (dev->media.type & DISC_DVD) return (int*)SPEEDS_JB_DVD;
			break;
		default:
			break;
	}
...
int scan_plextor::scan_block(void* data, uint32_t* ilba) {
	int r = -1;
	switch (test) {
		case CHK_ERRC_CD:
