...
int scan_plextor::cmd_cd_errc_block(cd_errc* data) {
	...
	for (int i = 0; (i < 5) && lba < dev->media.capacity; i++) {
		if (lba + 15 > dev->media.capacity) rsize = dev->media.capacity - lba;
		//		else
		//			rsize = 15;
...
int scan_plextor::build_TA_histogram_px755(unsigned char* response_data, int* dest_pit, int* dest_land, int len,
                                           int dt) { //, int spd) {
	int* dest[] = {dest_land, dest_pit};
	int count = ntoh16(response_data + 2);
...
int scan_plextor::cmd_dvd_ta_block(cdvd_ta* data) {
	...
	int i, r, m, j;
	float sum;
	unsigned char scan_cmd[6][2] = {{0x04, 0x00}, {0x10, 0x00}, {0x20, 0x00}, {0xFA, 0x28}, {0xEA, 0x28}, {0xDE, 0x28}};
	const char* scan_txt[] = {"Running TA on L0 inner zone ", "Running TA on L0 middle zone",
	...
	if (data->pass < 0 || data->pass > 6) return -1;
	...
	for (i = 0; i < 9; i++) {
		...
		printf(".\n");
		if (!strncmp(dev->dev, "DVDR   PX-714A", 14) || !strncmp(dev->dev, "DVDR   PX-716A", 14))
