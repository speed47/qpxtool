...
devctlCommon::devctlCommon(device* idev, QWidget* p) : QGroupBox(p) {
	...
	if (dev->features.supported & FEATURE_POWEREC) l_prec->setText(QString::number(dev->features.prec_spd) + "X");
...
devctlGigarec::devctlGigarec(device* idev, QWidget* p) : QGroupBox("GigaRec", p) {
	QString gc;
	...
	bool ratio_11 = dev->dev.startsWith("DVDR   PX-755") || dev->dev.startsWith("DVDR   PX-760") ||
	...
	l_ncap->setText(tr("GigaRec capacity: ") + QString::number((int)(dev->media.ctots * dev->features.grec) >> 9) +
	                " MB");
...
devctlVarirecBase::devctlVarirecBase(device* idev, QString title, QWidget* p) : QGroupBox(title, p) {
	dev = idev;
	layout = new QVBoxLayout(this);
	layout->setContentsMargins(3, 3, 3, 3);
	layout->setSpacing(3);
...
devctlVarirecCD::devctlVarirecCD(device* idev, QWidget* p) : devctlVarirecBase(idev, "Varirec CD", p) {
...
devctlVarirecDVD::devctlVarirecDVD(device* idev, QWidget* p) : devctlVarirecBase(idev, "Varirec DVD", p) {
	l_hint->setText(tr("VariRec limits maximum DVD write speed to 4X"));
	str->addItem("default");
	for (int s = 0; s < 8; s++) str->addItem(QString("Strategy %1").arg(s));
...
devctlSecurec::devctlSecurec(device* idev, QWidget* p) : QGroupBox("SecuRec", p) {
	dev = idev;
	...
	l_hint = new QLabel(tr("Password length must me between 4 and 10 characters.\n\n"
...
devctlAutostrategy::devctlAutostrategy(device* idev, QWidget* p) : QWidget(p) {
	...
	connect(grp_mode, SIGNAL(idClicked(int)), this, SLOT(set_mode(int)));
...
devctlAutostrategyDB::devctlAutostrategyDB(device* idev, QWidget* p) : QDialog(p) {
	dev = idev;
...
void devctlAutostrategyDB::db_update(int idx) {
	...
	for (int i = 0; i < dev->asdb.size(); i++) {
		if (dev->asdb[i].present) {
			it = new QTreeWidgetItem(tree, QStringList() << "" << dev->asdb[i].type << dev->asdb[i].mid
...
void devctlAutostrategyDB::db_act() { db_act(tree->indexOfTopLevelItem(tree->currentItem())); }
...
void devctlAutostrategyDB::db_deact() { db_deact(tree->indexOfTopLevelItem(tree->currentItem())); }
...
void devctlAutostrategyDB::db_del() {
	...
	if (idx < 0) return;
	if (QMessageBox::warning(this, tr("Are you sure?"), tr("Are you sure to delete strategy #%1?").arg(idx),
...
void devctlAutostrategyDB::db_clear() {
	if (QMessageBox::warning(this, tr("Are you sure?"), tr("Are you sure delete all strategies?"),
	                         QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel) != QMessageBox::Yes)
		return;
...
void devctlAutostrategyDB::run_ascre() {
	...
	if (dev->media.creads || !dev->media.type.startsWith("DVD") || dev->media.erasable == "yes") {
		QMessageBox::warning(this, tr("Can't create strategy!"),
	...
	dev->features.as_act_mode =
	    (rb_ascre_full->isChecked() ? ASCRE_FULL : 0) | (rb_ascre_replace->isChecked() ? ASCRE_REPLACE : 0);
	dev->startAScre();
...
devctlDestruct::devctlDestruct(device* idev, QWidget* p) : QGroupBox(tr("Data destruction"), p) {
	dev = idev;
...
void devctlDestruct::start() {
	ProgressWidget* progress;
	if (QMessageBox::warning(this, tr("Are you sure?"), tr("Media will be unreadable after this operation!"),
	                         QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel) != QMessageBox::Yes)
		return;
...
devctlPioquiet::devctlPioquiet(device* idev, QWidget* p) : QGroupBox(tr("Pioneer Quiet Mode"), p) {
	dev = idev;
...
devctlF1Tattoo::devctlF1Tattoo(device* idev, QWidget* p) : QGroupBox("Yamaha CRW-F1 Disc T@2", p) {
	dev = idev;
...
void devctlF1Tattoo::burn() {
	...
	for (int y = img.height() - 1; y >= 0; y--) {
		for (int x = 0; x < F1TATTOOW; x++) { line[x] = qGray(img.pixel(x, y)) ^ 255; }
		tf.write(line);
	}
	tf.close();
