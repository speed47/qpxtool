...
void ProgressWidget::paintEvent(QPaintEvent*) {
	...
	if (text.isEmpty()) {
		for (int i = 0; i < shown; i++)
			p.fillRect(offs + i * blkw, (height() - blkw) >> 1, (int)(blkw * 0.8), (int)(blkw * 0.8), QBrush(col_rect));
	} else {
		...
		p.drawText(0, 0, width(), height() >> 1, Qt::AlignHCenter | Qt::AlignVCenter, text);
		for (int i = 0; i < shown; i++)
			p.fillRect(offs + i * blkw, height() >> 1, (int)(blkw * 0.8), (int)(blkw * 0.8), QBrush(col_rect));
	}
}
