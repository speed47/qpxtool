...
ImageLabel::ImageLabel(int iminW, QString label, QImage image, int idx, QWidget* parent) : QWidget(parent) {
#ifndef QT_NO_DEBUG
	qDebug("ImageLabel()");
#endif
	iconw = 64;
...
ImageLabel::ImageLabel(int iminW, QSize is, QString label, QImage image, int idx, QWidget* parent) : QWidget(parent) {
#ifndef QT_NO_DEBUG
	qDebug("ImageLabel()");
#endif
	iconw = is.width();
...
void ImageLabel::paintEvent(QPaintEvent*) {
	...
	if (ck) p.fillRect(QRect(1, 1, width() - 2, height() - 2), QBrush(palette().color(QPalette::Highlight)));
