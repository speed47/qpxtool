...
static struct option long_options[] = {{"load", 1, NULL, 'l'}, {"help", 0, NULL, 'h'}, {0, 0, 0, 0}};

QPxToolMW::QPxToolMW(int ac, char** av, QWidget* p, Qt::WindowFlags fl) : QMainWindow(p, fl) {
	char c;
	QString lname;
	splash = 1;
	testDialog = NULL;
	...
	if (set.geometry_mw.width() > 0 && set.geometry_mw.height() > 0) setGeometry(set.geometry_mw);
	scanbus();

	if (!lname.isEmpty()) { load_results(lname); }
#ifndef QT_NO_DEBUG
...
//void QPxToolMW::run_tests(device *dev)
{
	...
	static const int test_tab_map[][2] = {
	    {TEST_RT, 2}, {TEST_WT, 3}, {TEST_ERRC, 4}, {TEST_JB, 5}, {TEST_FT, 6}, {TEST_TA, 7},
	};
...
void QPxToolMW::scanbus() {
	...
	for (int i = 0; i < devices.size(); i++) {
		if (!devices[i]->mutex->tryLock()) {
			for (int j = 0; j < i; j++) devices[j]->mutex->unlock();
			mutex_dev.unlock();
			QMessageBox::information(this, tr("Device busy"), tr("Can't scan bus: some device(s) busy!"));
			return;
		}
...
*/
	...
	//	scanbusDis = 0;
	if (!devices.size()) { c_dev->addItem(tr("No devices found!")); }
...
void QPxToolMW::scanbus_local() {
	...
	disconnect(proc);
	disconnect(scanbusio, SIGNAL(readyReadLine()), this, SLOT(qscan_process_scanbus()));
	scanbusio->setIODevice(NULL);
	delete proc;
scanbus_local_end:
#ifndef QT_NO_DEBUG
...
void QPxToolMW::scanbus_remote() {
	...
	for (int i = 0; i < hosts_cnt; i++) {
		...
		if (i < set.ports.size()) dport = set.ports[i].toInt();
...
//		qout.remove("\n");
		...
		if (qout.startsWith("D:")) {
			...
			if (dt == device::DevtypeLocal) {
				dev->id = QString("%1 %2 %3 [ %4 ]").arg(dev->ven).arg(dev->dev).arg(dev->fw).arg(dev->path);
...
void QPxToolMW::save_report() {
	...
	QString fname;
	QFileInfo finfo;
	fname = QFileDialog::getSaveFileName(this, tr("Export results to PDF"), set.last_res_path_pdf,
	                                     "PDF files (*pdf *.PDF)");
		...
		if (dev->media.type.startsWith("CD")) {
			...
			for (int i = 0; i < 3; i++) {
				if (!dev->media.tdata_errc || (dev->media.tdata_errc & (1 << residx[i]))) {
					...
					doc->addResource(QTextDocument::ImageResource, QUrl("images/" + errcNameBD[residx[i]] + ".png"),
					                 img);
				}
				qApp->processEvents();
			}
	...
	if (dev->test_req & TEST_TA) {
		for (int l = 0; l < dev->media.ilayers; l++) {
			for (int z = 0; z < 3; z++) {
				...
				doc->addResource(QTextDocument::ImageResource, QUrl(QString("images/ta%1_%2_p.png").arg(l).arg(z)),
				                 img);
				...
				doc->addResource(QTextDocument::ImageResource, QUrl(QString("images/ta%1_%2_l.png").arg(l).arg(z)),
				                 img);
...
void QPxToolMW::export_results() {
	...
	fname =
	    QFileDialog::getSaveFileName(this, tr("Export results to HTML"), set.last_res_path_html, "HTML files (*.html)");
	if (fname.isEmpty()) return;
...
QString QPxToolMW::generate_html(device* dev, QString idir) {
	...
	r += "\t\t\t<td>" + QString::number(dev->media.creadm) + "MB (" + QString::number(dev->media.creads) +
	     " sectors)</td>\n";
	r += "\t\t</tr>\n";
	r += "\t\t<tr>\n";
	r += "\t\t\t<td>Capacity (total):</td>\n";
	r += "\t\t\t<td>" + QString::number(dev->media.ctotm) + "MB (" + QString::number(dev->media.ctots) +
	     " sectors)</td>\n";
	r += "\t\t</tr>\n";
	r += "\t</table>\n";
	r += "\t<br>\n";
	...
	if (dev->testData.rt.size() > 0) {
		...
		r += QString("\t<tr bgcolor=#e0e0e0><td align=right>%1 X</td><td align=right>%2 X</td><td align=right>%3 "
		             "X</td></tr>\n")
		...
		r += QString("\t<tr bgcolor=#c0c0c0><td align=right>%1 kB/s</td><td align=right>%2 kB/s</td><td align=right>%3 "
		             "kB/s</td></tr>\n")
	...
	if (dev->testData.wt.size() > 0) {
		...
		r += "<table border=0 cellspacing=0 align=\"center\">\n\t<tr><td align=\"center\">";
		r +=
		    "<img src=\"" + idir + "/wt.png\" alt=\"Write Transfer\" width=" HTML_GRAPH_WQ " height=" HTML_GRAPH_HQ ">";
		//		r+= "<br>Write Transfer";
		r += "</td></tr>\n</table><br>\n";
	}
	...
	if (dev->testData.errc.size() > 0) {
		...
		if (dev->media.type.startsWith("CD")) {
			for (int i = 0; i < 8; i++) {
				if (!dev->media.tdata_errc || (dev->media.tdata_errc & (1 << i))) {
					...
					r += QString("\t\t<tr bgcolor=%1><td>%2</td><td align=right>%3</td><td align=right>%4</td><td "
					             "align=right>%5</td></tr>\n")
					         .arg(bgcolor)
					         .arg(errcNameCD[i])
		...
		r += "<table border=0 cellspacing=0 align=\"center\">\n\t<tr><td align=\"center\">";
		r += "<img src=\"" + idir +
		     "/errc.png\" alt=\"Error correction\" width=" HTML_GRAPH_WQ " height=" HTML_GRAPH_HQ ">\n";
	...
	if (dev->test_req & TEST_TA) {
		...
		for (int l = 0; l < dev->media.ilayers; l++) {
			for (int z = 0; z < 3; z++) {
				r += "<table border=0 cellspacing=0 align=center>\n\t<tr><td align=center>";
				r += "<img src=\"" + idir + "/ta" + QString::number(l) + "_" + QString::number(z) +
...
void QPxToolMW::save_results() {
	...
	fname = QFileDialog::getSaveFileName(this, tr("Save results to file..."), set.last_res_path_native,
...
void QPxToolMW::load_results() {
	...
	QFileInfo finfo;
	if (!devices.size()) return;
	fname = QFileDialog::getOpenFileName(this, tr("Load results from file..."), set.last_res_path_native,
	                                     "QPxTool data files (*.qpx)");
	if (fname.isEmpty()) return;
...
void QPxToolMW::dragEnterEvent(QDragEnterEvent* e) {
	...
	if (e->mimeData()->hasFormat("text/plain")) {
		...
		if (url.path().endsWith(".qpx")) e->acceptProposedAction();
	}
}
