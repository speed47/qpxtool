...
DbReportSelection::DbReportSelection(QString conn, QWidget* parent, Qt::WindowFlags fl) : QDialog(parent, fl) {
	report_id = -1;
	...
	while (q->next()) {
		...
		if (!q->value(3).toInt())
			...
			model->addRow(q->value(0).toInt(),
			              q->value(2).toString() + ", " + tr("copy") + " #" + q->value(3).toString(), icon);
	}
	list->setModel(model);
...
void DbReportSelection::itemActivated(const QModelIndex idx) {
	...
	info->document()->addResource(
	    QTextDocument::ImageResource, QUrl("images//:media_logo.png"),
	...
	if (!md5.isEmpty()) { html += "MD5: " + md5; }
	html += "</td></tr>";

	html += "<tr><td bgcolor=#E0E0E0>" + tr("Device") + "</td><td>" + q->value(3).toString() + q->value(4).toString() +
