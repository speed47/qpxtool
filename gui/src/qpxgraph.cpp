...
#ifndef QT_NO_DEBUG
//	qDebug(QString("QPxGraph::wheelEvent(): %1").arg(e->angleDelta().y()));
#endif
...
void QPxGraph::drawGraph(QPainter* p, QSize s, device* dev, int ttype, const QRect& rect, int eflags,
	...
	QSize sg;
	QRect grect =
	    rect.intersected(geometry().adjusted(margin_left, 0, -margin_right, -margin_bottom)).translated(margin_left, 0);

	forceAll |= forceRepaint;

	if (forceRepaint) p->fillRect(rect, settings->col_bg);
	...
	if (ttype == TEST_TA) {
		...
		if (forceAll) p->fillRect(1, 1, width() - 2, height() - margin_bottom - 2, QBrush(settings->col_bg));
	} else {
		sg = QSize(s.width() - (margin_left + margin_right + 2), s.height() - margin_bottom - 1);
		if (forceAll || margin_left > rect.x())
			p->fillRect(margin_left + 1, 1, width() - (margin_left + margin_right + 2), height() - margin_bottom - 1,
			            QBrush(settings->col_bg));
		else
			p->fillRect(rect.x(), 1, width() - rect.x() - margin_left, height() - margin_bottom - 1,
			            QBrush(settings->col_bg));
...
#endif
					// update min/max
					if (min < 0 || min > dev->testData.errc[i].raw.err[e]) min = dev->testData.errc[i].raw.err[e];
					if (max < dev->testData.errc[i].raw.err[e]) max = dev->testData.errc[i].raw.err[e];
				}

				if (x != xo || i == (dev->testData.errc.size() - 1)) {
					// min-max
					p->setPen(QPen(*settings->col_errc.raw[e], 1));
					p->drawLine(xo, errc2h(s.height(), min), xo, errc2h(s.height(), max));
...
// P=0.95
					...
					p->drawLine(xo, errc2h(s.height(), M - D), xo, errc2h(s.height(), M + D));
					xerr.clear();
#endif
					min = -1;
					max = -1;
...
// draw speed
		...
		qDebug() << QString("draw time(sec): %1  ERRC,  %2  Speed").arg(t0, 0, 'f', 6).arg(t1, 0, 'f', 6);
#endif
	}
	...
	for (i = startidx; i < dev->testData.jb.size(); i++) {
		...
		if (dev->testData.jb[i].jitter >= -100 && dev->testData.jb[i].asymm >= -100) {
			...
			if (first) {
				...
				if (jmin > dev->testData.jb[i].jitter) jmin = dev->testData.jb[i].jitter;
				if (jmax < dev->testData.jb[i].jitter) jmax = dev->testData.jb[i].jitter;
				if (bmin > dev->testData.jb[i].asymm) bmin = dev->testData.jb[i].asymm;
				if (bmax < dev->testData.jb[i].asymm) bmax = dev->testData.jb[i].asymm;
			}
		}
...
#ifndef QT_NO_DEBUG
			qDebug() << QString("i=%1   J: %2..%3   A: %4..%5").arg(i).arg(jmin).arg(jmax).arg(bmin).arg(bmin);
#endif
			// ******* Jitter
			// min-max
			p->setPen(QPen(settings->col_jitter, 1));
			p->drawLine(xo, jitter2h(s.height(), jmin), xo, jitter2h(s.height(), jmax));
// P=0.95
#ifdef SHOW_P95JB
			M = xj.M();
			D = sqrt(xj.dispers(M));
			p->setPen(QPen(settings->col_jitter.darker(), 1));
			p->drawLine(xo, jitter2h(s.height(), (M - D) / 100.0), xo, jitter2h(s.height(), (M + D) / 100.0));
			xj.clear();
#endif
...
// P=0.95
			...
			p->drawLine(xo, asymm2h(s.height(), (M - D) / 100.0), xo, asymm2h(s.height(), (M + D) / 100.0));
			xb.clear();
#endif
			xo = x;
...
*/
	...
	for (int i = 1; i < VGRIDS; i++)
		p->drawLine(margin_left + (int)(i * HscaleX), s.height() - margin_bottom - 1, margin_left + (int)(i * HscaleX),
		            1);
	...
	if (isCD) {
		for (int i = 1; i < VGRIDS; i++) {
			if (!(i & 1))
				p->drawText(margin_left + (int)(HscaleX * (i - 1)), s.height() - margin_bottom + 2, (int)(HscaleX * 2),
	...
	switch (ttype) {
		...
		case TEST_WT:
			for (int i = (isCD ? 4 : 2); i < spdMax; i += (isCD ? 4 : 2)) {
				...
				if (!(i & (isCD ? 7 : 1))) {
					p->setPen(QPen(palette().color(QPalette::Text), 1));
					p->drawText(2, h - 10, margin_left - 4, 20, Qt::AlignVCenter | Qt::AlignRight, QString::number(i));
					p->setPen(QPen(settings->col_grid, 1, GRID_STYLE));
				}
			}
			break;
		case TEST_ERRC:
			if (scale[0]->type == Scale::Log) {
				...
				for (int i = 0; errc_logh[i] > 0; i++) {
					h = errc2h(s.height() - margin_bottom, errc_logh[i]);
					p->drawText(2, h - 10, margin_left - 4, 20, Qt::AlignVCenter | Qt::AlignRight,
...
//				qDebug(QString("scale step: %1").arg(step));
			...
			if (!label.isEmpty()) {
				...
				p->drawText(margin_left + 5, 4, 60, 20, Qt::AlignTop | Qt::AlignLeft, label);
				p->setFont(dfont);
			}
			// error limit hlines
...
void QPxGraph::drawGridTA(QPainter* p, const QSize& s, device* dev, int ttype) {
	...
	// vertical grid lines
	for (int i = 3; i <= 11; i++)
		p->drawLine((int)((i - 2) * Hscale), s.height() - margin_bottom, (int)((i - 2) * Hscale), 0);

	if (dev->media.type.startsWith("DVD")) {
		p->drawLine((int)(12 * Hscale), s.height() - margin_bottom, (int)(12 * Hscale), 0);
	}
	// bottom text labels
	p->setPen(QPen(palette().color(QPalette::Text), 1));
	for (int i = 3; i <= 11; i++) {
		p->drawText((int)(Hscale * (i - 2.5)), s.height() - margin_bottom + 2, (int)(Hscale), margin_bottom - 2,
		            Qt::AlignVCenter | Qt::AlignHCenter, QString("T%1").arg(i));
	}
	if (dev->media.type.startsWith("DVD")) {
		p->drawText((int)(Hscale * (11.5)), s.height() - margin_bottom + 2, (int)(Hscale), margin_bottom - 2,
...
void QPxGraph::contextMenuEvent(QContextMenuEvent* e) {
	...
	cmenu->addAction((scale[0]->type == Scale::Log) ? icon_ok : QIcon(), tr("Logarithmic scale"), this,
	                 SLOT(setScaleTypeLog()));
	cmenu->addAction((scale[0]->type == Scale::Linear) ? icon_ok : QIcon(), tr("Linear Scale"), this,
	                 SLOT(setScaleTypeLin()));
	cmenu->addSeparator();
