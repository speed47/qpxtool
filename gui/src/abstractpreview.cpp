...
#endif
	for (int i = tiles.size(); i > 0; i--) delete tiles.takeLast();
	tiles.clear();
}
...
void AbstractPreviewPrivate::setup() {
	...
	paperSize = QSize(inchesToPixelsX(mmToInches(paperSizeMM.width()), currentDevice) + 1,
	...
	pageSizeMM = QSize(paperSizeMM.width() - mm_left - mm_right, paperSizeMM.height() - mm_top - mm_bottom);
#else
	pageSizeMM = printer->pageLayout().paintRect(QPageLayout::Millimeter).size();
#endif

	pageSize = QSize(inchesToPixelsX(mmToInches(pageSizeMM.width()), currentDevice) + 1,
	...
	if (!preview->horizontalScrollBar()->value()) {
		if (preview->viewport()->width() < paperWidthToScale())
			...
			if (cols == 1)
				...
				xx = (preview->viewport()->width() - (paperWidthToScale() * cols + interPageSpacing * (cols - 1))) / 2;
		}
	}
...
void AbstractPreviewPrivate::clear() {
	flushCache();
	while (pagesData.count()) { delete pagesData.takeLast(); }
}
//--------------------------------------------------------------
AbstractPreview::AbstractPreview(QWidget* parent, QPrinter* printer) : QAbstractScrollArea(parent) {
...
//--------------------------------------------------------------
AbstractPreview::AbstractPreview(QWidget* parent, QPrinter* printer, int _countPage) : QAbstractScrollArea(parent) {
	setMouseTracking(true);
...
int AbstractPreview::sizeToCountPage() {
	...
	int h = qRound(rows * (d->paperHeightToScale() + d->pageLabelHeight) + (rows - 1.) * d->interPageSpacing +
	               2. * d->pageMargin);
...
//	qDebug() << "Tile not found in cache, rendering new one";
		...
		pix = new QPixmap(qMin(d->tileWidth, (d->pageSizeScaled.width() - d->tileWidth * tx)),
...
void AbstractPreview::updatePage(int page, QRect rect) {
	...
	const int yy2 = yy + d->pageSizeScaled.height(), xx2 = xx + d->pageSizeScaled.width();
	...
	for (int ty = ty_sta; ty < ty_end; ty++) {
		for (int tx = tx_sta; tx < tx_end; tx++) {
			QRect tileRect(d->tileWidth * tx, d->tileHeight * ty, d->tileWidth, d->tileHeight);
			QRect sec = tileRect.intersected(rect);
			if (!sec.isValid()) continue;
			QPixmap* px = getTile(page, tx, ty);
			if (!px) continue;
			QPainter pp(px);
			pp.drawPixmap(sec.topLeft() - tileRect.topLeft(), pix, QRect(sec.topLeft() - rect.topLeft(), sec.size()));
		}
	}
...
void AbstractPreview::paintEvent(QPaintEvent*) {
	...
	for (int y = 0; y < d->countPagesRow; y++) {
		...
		for (int x = 0; x < d->cols; x++) {
			...
			qDebug() << "Page: " << page << "  " << tx_sta << "." << ty_sta << " - " << tx_end << "." << ty_end
			         << QString("  time: %1 s")
			                .arg(te.tv_sec - tb.tv_sec + (te.tv_usec - tb.tv_usec) / 1000000.0, 0, 'f', 4);
#endif
			...
			QPageInfo pg(page, QRect(xx - d->ms_left, yy - d->ms_top, d->paperWidthToScale(), d->paperHeightToScale()));
		...
		if (d->viewMode != Mode_Thumbs) {
			int _y = yy - d->ms_top;
			if (_y < (viewport()->height() >> 1)) { d->selectPageIndex = page - 1; }
		}
	...
	if (d->viewMode != Mode_Thumbs && d->selectPageIndex >= 0) { emit currentPage(d->selectPageIndex); }
}
//--------------------------------------------------------------
...
QPoint AbstractPreview::toPage(QPoint point) {
	const int count = d->pagesRect.count();
	for (int i = 0; i < count; i++) {
		if (d->pagesRect[i].rect().contains(point)) { return point - d->pagesRect[i].rect().topLeft(); }
	}
...
QPageInfo AbstractPreview::pageInfo(int page) {
	if (d->enterPageInfo.isValid() && d->enterPageInfo.page() == page) return d->enterPageInfo;
...
void AbstractPreview::moveContext(const QPoint& point) {
	...
	horizontalScrollBar()->setValue(d->scrollBarValueOnMousePress.x() - point.x() + d->mousePressData.pos.x());
	verticalScrollBar()->setValue(d->scrollBarValueOnMousePress.y() - point.y() + d->mousePressData.pos.y());
	d->calcParamDraw();
...
void AbstractPreview::mouseMoveEvent(QMouseEvent* e) {
	if (!d->mousePressData.press) {
		pageInfo(e->pos());
		if (d->enterPageInfo.isValid()) mouseMovePageEvent(e, d->enterPageInfo);

		e->ignore();
		return;
	}

	if (d->movedContents) moveContext(e->pos());
}
//--------------------------------------------------------------
...
//--------------------------------------------------------------
void AbstractPreview::mousePressPageEvent(QMouseEvent* event, int page, QRect rect, QPoint point) {
	Q_UNUSED(event);
	Q_UNUSED(page);
	Q_UNUSED(rect);
	Q_UNUSED(point);
...
void AbstractPreview::setMargins(qreal ml, qreal mt, qreal mr, qreal mb) {
	if (d->mm_left == ml && d->mm_right == mr && d->mm_top == mt && d->mm_bottom == mb) return;
	d->mm_left = ml;
	d->mm_right = mr;
	d->mm_top = mt;
	d->mm_bottom = mb;
...
void AbstractPreview::print() {
	...
	if (dlg->exec() == QDialog::Accepted) { printPages(d->printer); }
	delete dlg;
#ifndef QT_NO_DEBUG
...
#ifdef DEBUG_PRINT_TIME
	qDebug() << "Setup time: "
	         << QString(" %1 s").arg(tpb.tv_sec - tsb.tv_sec + (tpb.tv_nsec - tsb.tv_nsec) / 1000000000.0, 0, 'f', 4);
#endif
#endif

#ifdef DEBUG_PRINT_TIME
	qDebug() << "Pages time: "
	         << QString(" %1 s").arg(tpe.tv_sec - tpb.tv_sec + (tpe.tv_nsec - tpb.tv_nsec) / 1000000000.0, 0, 'f', 4);
#endif
}
//--------------------------------------------------------------
...
#endif
	d->setup();
	if (d->viewMode == Mode_Thumbs) { autoscaleThumbs(); }
	updatePageFormat();
	d->changeScale();
	sizeToCountPage();
	d->calcParamDraw();
...
//--------------------------------------------------------------
QPixmap* AbstractPreview::grabPage(int page, qreal left, qreal top, qreal width, qreal height) {
	if (!(page >= 0 && page < pageCount())) return NULL;
	return NULL;
}
//--------------------------------------------------------------
