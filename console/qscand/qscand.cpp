...
#endif
{
	if (!childl) {
		...
		if (debug && !daemonized) printf("%d: SIGINT. Terminating client connection...\n", (int)pid);
	}
	term = 1;
#if defined(_WIN32) || defined(_WIN64)
	return 1;
...
//	signal(SIGCHLD, &sigchld_handler);
	...
	if (!iface || !inet_aton(iface, &srvaddr.sin_addr)) { srvaddr.sin_addr.s_addr = ip; }
	...
	for (; !term;) {
		...
		if (sret > 0) {
			if (FD_ISSET(listenfd, &rd_set)) {
				// #if !defined (_WIN32) && !defined (_WIN64)
				children[cli_idx].arg.connfd =
				    accept(listenfd, (struct sockaddr*)&children[cli_idx].arg.cliaddr, &cliaddr_len);
				/*
#else
...
// #if defined (_WIN32) || defined (_WIN64)
		...
		if (children[cli_idx].arg.connfd > 0) {
			...
			if (!child_create(cli_idx)) {
				...
				clients++;
				cmutex->unlock();
				if (debug && !daemonized) printf("%d: Clients: %d\n", (int)pid, clients);
	...
	if (debug && !daemonized) printf("%d: Closing listen socket...\n", (int)pid);
	close(listenfd);
