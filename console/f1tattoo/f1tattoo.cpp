	...
	if ((flags & (FL_CURRENT | FL_TATTOO | FL_TATTOO_TEST)) && (drive->ven_features & YMH_TATTOO)) {
		if (drive->yamaha.tattoo_rows) {
			printf("DiscT@2 info:\ninner: %d\nouter: %d\nimage: 3744x%d\n", drive->yamaha.tattoo_i,
			       drive->yamaha.tattoo_o, drive->yamaha.tattoo_rows);
		} else {
...
bool tattoo_read_png(unsigned char* buf, int rows, FILE* fp) {
	...
	for (row = 0; row < rows; row++) {
		...
		if (my_png_get_image_width(png_ptr, info_ptr) < 3744) memset(raw_row_pointer, 0, 3744);
...
int main(int argc, char* argv[]) {
	...
	for (i = 1; i < argc; i++) {
		...
		if (!strcmp(argv[i], "-d")) {
			...
			flags |= FL_VERBOSE;
		else if (!strcmp(argv[i], "--tattoo-test"))
