...
static int initCSSCache(drive_info* dev) {
	...
	if (psz_home == NULL) psz_home = getenv("USERPROFILE");
	...
	if (psz_cache) {
		...
		if (!read(dev, p_sector, 0, 1)) { goto nocache; }

		if (p_sector[0] == 0x00 && p_sector[1] == 0x00 && p_sector[2] == 0x01 && p_sector[3] == 0xba) { goto nocache; }
		...
		if (!read(dev, p_sector, 16, 1)) { goto nocache; }
		...
		for (i = 0; i < 16; i++) {
			if (psz_serial[i] < '0' || psz_serial[i] > '9') {
				char psz_tmp[16 + 1];
				sprintf(psz_tmp, "%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x", psz_serial[0], psz_serial[1], psz_serial[2],
				        psz_serial[3], psz_serial[4], psz_serial[5], psz_serial[6], psz_serial[7]);
		...
		if (dev->media.dvdcss.protection == 0x01) {
			psz_key[0] = '-';
			for (i = 0; i < DVD_KEY_SIZE; i++) { sprintf(&psz_key[1 + i * 2], "%.2x", dev->media.dvdcss.DK[i]); }
		...
		i += sprintf(dev->media.dvdcss.psz_cachefile + i, "/%s-%s%s", psz_title, psz_serial, psz_key);
		if (mkdir(dev->media.dvdcss.psz_cachefile, 0755) < 0 && errno != EEXIST) {
...
*/
	...
	if (!dev->silent) { printf("libdvdread: Attempting to retrieve all CSS keys\n"); }
	...
	for (title = 0; title < 100; title++) {
		...
		if (start != 0 && len != 0) {
			...
			//	  st0=1;
			if (!dev->silent) { printf("libdvdread: Get key for %s at 0x%08x, len %10u\n", filename, start, len); }
			if (css_title(dev, (int)start) < 0) {
				if (!dev->silent) { printf("libdvdread: Error cracking CSS key for %s (0x%08x)\n", filename, start); }
			}
			clock_gettime(CLOCK_MONOTONIC, &t_e);
			if (!dev->silent) { printf("libdvdread: Elapsed time %jd\n", (intmax_t)t_e.tv_sec - t_s.tv_sec); }
		}
	...
	if (!dev->silent) { printf("libdvdread: Found %d VTS's\n", title); }
	...
	if (!dev->silent) {
		...
		//		&& p_title->p_next != NULL
		{
			printf("STA: %8X,  KEY: %02X:%02X:%02X:%02X:%02X\n", p_title->i_startlb, p_title->p_key[0],
			       p_title->p_key[1], p_title->p_key[2], p_title->p_key[3], p_title->p_key[4]);
...
void* read_disc(void* arg) {
	...
	if (dev->media.type & DISC_CD) {
		...
		if (!dev->media.dvdcss.protection) {
			...
			if (dev->media.dvdcss.regmask != 0xFF) {
				for (int i = 0; i < 8; i++)
					if (!((dev->media.dvdcss.regmask >> i) & 1)) { printf("%d", i + 1); /*dev->rpc.region = i+1;*/ }
				printf("\n");
			} else {
	...
	if (dev->media.dvdcss.protection == 0x01) {
		...
		printf("\nSetting title key: %02X:%02X:%02X:%02X:%02X  for %x-%x, CSS=%d\n", dev->media.dvdcss.TK[0],
		       dev->media.dvdcss.TK[1], dev->media.dvdcss.TK[2], dev->media.dvdcss.TK[3], dev->media.dvdcss.TK[4], 0,
		       title_next, descramble);
	}
	...
	while (lba < dev->media.capacity) {
			...
			if (dev->media.dvdcss.protection == 0x01) {
				if (lba >= title_next && p_title) {
					...
					if (!memcmp(dev->media.dvdcss.TK, "\0\0\0\0\0", 5))
						...
						descramble = 1;
					printf("\nSetting title key: %02X:%02X:%02X:%02X:%02X  for %x-%x, CSS=%d\n",
					       dev->media.dvdcss.TK[0], dev->media.dvdcss.TK[1], dev->media.dvdcss.TK[2],
					       dev->media.dvdcss.TK[3], dev->media.dvdcss.TK[4], lba, title_next, descramble);
					p_title = p_title->p_next;
				}
		...
		printf("%s: %5.2f X, lba %7u / %7d  ( %06x / %06x ) scnt=%d\r", dev->device,
		       ((float)scnt / (float)sects1X) /
		           ((t_e.tv_sec - t_s.tv_sec) + (t_e.tv_nsec - t_s.tv_nsec) / 1000000000.0),
		...
		if (!ctry) {
			...
			if (parm->pass < PASS_RECOVER) map->set(lba, BM_FAIL, scnt);
