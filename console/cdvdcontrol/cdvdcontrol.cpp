...
int get_device_info(drive_info* dev) {
	...
	if ((dev->ven_features & YMH_TATTOO) && (flags & FL_CURRENT)) {
		if (dev->yamaha.tattoo_rows) {
			printf("Yamaha DiscT@2      : inner %d   outer %d   image 3744 x %d\n", dev->yamaha.tattoo_i,
			       dev->yamaha.tattoo_o, dev->yamaha.tattoo_rows);
		} else {
	...
	if ((dev->ven_features & PX_POWEREC) && ((flags & FL_CURRENT) || (flags & FL_POWEREC))) {
		...
		if (dev->media.type & DISC_CD) printf("      PoweRec Speed : %dX (CD)\n", dev->plextor.powerec_spd / 176);
		if (dev->media.type & DISC_DVD) printf("      PoweRec Speed : %dX (DVD)\n", dev->plextor.powerec_spd / 1385);
		//		show_powerec_speed();
	}
	if ((dev->ven_features & PX_GIGAREC) && ((flags & FL_CURRENT) || (flags & FL_GIGAREC))) print_gigarec_value(dev);
	...
	if (dev->ven_features & PX_ERASER) printf("PlexEraser          : supported\n");
...
void usage(char* bin) {
	...
	printf(
	    "\t--pio-quiet [quiet|perf|std] select QuietMode setting: Quiet, Performance or Standard (default: quiet)\n");
		...
		//  ************   Lock
			...
			if (argc > (i + 1)) {
				...
				if (!strcmp(argv[i], "off"))
					...
					spdread = 1;
				else { printf("Illegal argument for SpeedRead: %s\n", argv[i]); }
			} else {
				printf("option %s needs parameter!\n", argv[i]);
				return 5;
			}
			...
			if (argc > (i + 1)) {
				...
				if (!strcmp(argv[i], "off"))
					...
					sss = 1;
				else { printf("Illegal argument for SingleSession: %s\n", argv[i]); }
			} else {
				printf("option %s needs parameter!\n", argv[i]);
				return 5;
			}
			...
			if (argc > (i + 1)) {
				...
				if (!strcmp(argv[i], "off"))
					...
					hcdr = 1;
				else { printf("Illegal argument for Hide-CDR: %s\n", argv[i]); }
			} else {
				printf("option %s needs parameter!\n", argv[i]);
				return 5;
			}
		...
		//  ************   PoweREC
			...
			if (argc > (i + 1)) {
				...
				if (!strcmp(argv[i], "off"))
					...
					powerec = 1;
				else { printf("Illegal argument for PoweRec: %s\n", argv[i]); }
			} else {
				printf("option %s needs parameter!\n", argv[i]);
				return 5;
			}
		...
		//  ************   GigaREC
			...
			if (argc > (i + 1)) {
				...
				if (!strcmp(argv[i], "0.6"))
					...
					gigarec = GIGAREC_14;
				else { printf("Illegal GigaREC value: %s\n", argv[i]); }
			} else {
				printf("option %s needs parameter!\n", argv[i]);
				return 5;
			}
		...
		//  ************   VariREC  CD
			...
			if (argc > (i + 1)) {
				...
				if (!errno && val >= -1 && val < 6) {
					...
					varirec_cd_str = 6;
				else { printf("Illegal VariREC CD strategy: %s\n", argv[i]); }
			} else {
				printf("option %s needs parameter!\n", argv[i]);
				return 5;
			}
		...
		//  ************   VariREC  DVD
			...
			if (argc > (i + 1)) {
				...
				if (!errno && val >= -1 && val < 8) {
					...
					varirec_dvd_str = 0;
				else { printf("Illegal VariREC DVD strategy: %s\n", argv[i]); }
			} else {
				printf("option %s needs parameter!\n", argv[i]);
				return 5;
			}
...
*/
	...
	if (flags & FL_HCDR) { plextor_set_hidecdr(dev, hcdr); }
	if (flags & FL_SSS) { plextor_set_singlesession(dev, sss); }
	if (flags & FL_SPDREAD) { plextor_set_speedread(dev, spdread); }
	if (flags & FL_BOOK_R) {
	...
	if (flags & FL_MQCK) {
		...
		if (as_mqck >= 0) {
			dev->parms.max_write_speed_kb = (int)(as_mqck_spd * dev->parms.speed_mult), set_rw_speeds(dev);
			get_rw_speeds(dev);
	...
	if (flags_as & FLAS_STOR) {
		...
		if (!strncmp((char*)hdr, "ASDB ", 5)) dev->astrategy.dbcnt = hdr[5];
		i = 0;
	...
	// PIONEER features
	if (flags & FL_PIOQUIET) { pioneer_set_silent(dev, piosilent, silent_save); }
	if (flags & FL_PIOLIMIT) { pioneer_set_spdlim(dev, piolimit, silent_save); }
