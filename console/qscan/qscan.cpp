...
void detect_vendor_features(drive_info* dev) {
	if (isPlextor(dev)) {
		...
		//if ((dev->dev_ID == PLEXTOR_755) || (dev->dev_ID == PLEXTOR_760) || (dev->dev_ID == PLEXTOR_PREMIUM2))
		if (isPlextorLockPresent(dev)) dev->ven_features |= PX_ERASER;
#if 0
...
void show_available_errc_data(qscanner* scanner) {
	...
	if (dev->media.type & DISC_CD) {
		for (int i = 0; i < 8; i++)
			if (errc_data & (1 << i)) printf(" %s", errc_names_cd[i]);
	} else if (dev->media.type & DISC_DVD) {
		for (int i = 0; i < 8; i++)
			if (errc_data & (1 << i)) printf(" %s", errc_names_dvd[i]);
...
*/
	...
	while (1) {
		...
		switch (c) {
			...
			case 'd':
				...
				break;
			case 'i':
				if (!device) { printf(MSGPREF "no device selected!\n"); }
			...
			case 'l':
				...
				return 0;
			case 't':
				if (!(strcmp(optarg, "rt") && strcmp(optarg, "wt") && strcmp(optarg, "errc") && strcmp(optarg, "jb") &&
	...
	if (!liteon_force_old) {
		...
		if (env && strcmp(env, "1") == 0) liteon_force_old = true;
	}
	...
	if (flags & (FL_INFO | FL_MINFO)) {
		...
		printf(IDEV "Current read speed  : %4.1fX  ( %5d kB/s)\n",
		       (float)dev->parms.read_speed_kb / dev->parms.speed_mult, dev->parms.read_speed_kb);
		if (dev->wr_capabilities) {
			printf(IDEV "Current write speed : %4.1fX  ( %5d kB/s) \n",
			       (float)dev->parms.write_speed_kb / dev->parms.speed_mult, dev->parms.write_speed_kb);
		}
	}

	if (dev->wr_capabilities) { check_write_modes(dev); }
	if (flags & FL_INFO) {
...
//			printf("Yamaha ForceSpeed   : %s\n", drive->yamaha.forcespeed ? "ON":"OFF");
		...
		if (dev->wr_capabilities & DEVICE_DVD) {
			...
			if ((dev->ven_features & PX_VARIREC_DVD) && (dev->plextor.varirec_state_dvd))
	...
	if ((flags & (FL_SPEEDS | FL_MINFO)) && dev->media.type) {
		...
		if (!(dev->media.type & (DISC_CDROM | DISC_DDCD_ROM | DISC_DVDROM | DISC_BD_ROM | DISC_HDDVD_ROM))) {
			...
			printf(SMEDIA "D WR speed max: %4.1fX (%d kB/s)\n",
			       ((float)dev->parms.max_write_speed_kb) / dev->parms.speed_mult, dev->parms.max_write_speed_kb);
	...
	if (!pname && ((test && (strcmp(test, "rt") && strcmp(test, "wt"))) || (flags & (FL_MINFO | FL_LPLUGIN)))) {
		...
		if (scanner->plugins_probe(!test && (flags & FL_LPLUGIN), false) && !(flags & FL_LPLUGIN)) {
			...
			if (nprobed > 0) {
				...
				if (nprobed > 1) { printf(MSGPREF "%d compatible plugins found by probing\n", nprobed); }
				scanner->plugin_attach(probed[0].name);
			}

			if (!scanner->is_plugin_attached()) {
				printf(MSGPREF "Probe failed! Trying fallback plugin...\n");
				if (scanner->plugin_attach_fallback()) { printf(MSGPREF "Error loading fallback plugin!\n"); }
			}
		}
	...
	if (flags & FL_MINFO) {
		...
		if (scanner && scanner->is_plugin_attached()) {
			printf(IMEDIA "Available quality tests:%s%s%s%s%s%s\n", dev->media.capacity ? " rt" : "",
			       dev->media.dstatus ? "" : " wt", scanner->check_test(CHK_ERRC) ? "" : " errc",
			       scanner->check_test(CHK_JB) ? "" : " jb", scanner->check_test(CHK_FETE) ? "" : " ft",
		...
		if (dev->media.type & DISC_BD) printf(IMEDIA "GB Per Layer  : %d\n", dev->media.gbpl);
...
*/
		...
		printf(IMEDIA "Read capacity : %d sectors/%dMB/%02d:%02d.%02dMSF\n", dev->media.capacity,
		       dev->media.capacity / 512, dev->media.capacity / 4500, ((dev->media.capacity) / 75) % 60,
		       dev->media.capacity % 75);
		printf(IMEDIA "Free capacity : %d sectors/%dMB/%02d:%02d.%02dMSF\n", dev->media.capacity_free,
		       dev->media.capacity_free / 512, dev->media.capacity_free / 4500, ((dev->media.capacity_free) / 75) % 60,
		       dev->media.capacity_free % 75);
		printf(IMEDIA "Total capacity: %d sectors/%dMB/%02d:%02d.%02dMSF\n", dev->media.capacity_total,
		       dev->media.capacity_total / 512, dev->media.capacity_total / 4500,
		       ((dev->media.capacity_total) / 75) % 60, dev->media.capacity_total % 75);
		...
		if (dev->media.type & (DISC_DVDRAM | DISC_HDDVD_RAM)) {
			...
			printf(IMEDIA "Primary SA    : %d (%dMB) / %d (%dMB)\n", dev->media.spare_psa_free,
			       dev->media.spare_psa_free / 512, dev->media.spare_psa_total, dev->media.spare_psa_total / 512);
			printf(IMEDIA "Supplement. SA: %d (%dMB) / %d (%dMB)\n", dev->media.spare_ssa_free,
			       dev->media.spare_ssa_free / 512, dev->media.spare_ssa_total, dev->media.spare_ssa_total / 512);
		}

		if (!(dev->media.type & (DISC_CDROM | DISC_DVDROM))) printf(IMEDIA "Media ID      : %s\n", dev->media.MID);
		if (dev->media.type & (DISC_DVDminus)) printf(IMEDIA "Written on    : %s\n", dev->media.writer);
		if ((flags & FL_MID_RAW) && (dev->media.type & DISC_CD) && dev->media.ATIP_size) {
			for (int i = 0; i < ((dev->media.ATIP_size - 4) >> 2); i++) {
				...
				printf(" %02X %02X %02X %02X", dev->media.ATIP[(i << 2) + 4], dev->media.ATIP[(i << 2) + 5],
				       dev->media.ATIP[(i << 2) + 6], dev->media.ATIP[(i << 2) + 7]);
				printf(" | %3d %3d %3d %3d\n", dev->media.ATIP[(i << 2) + 4], dev->media.ATIP[(i << 2) + 5],
				       dev->media.ATIP[(i << 2) + 6], dev->media.ATIP[(i << 2) + 7]);
			}
		}
		...
		printf(IMEDIA
		       "Track:     Session   TMode   DMode      Start        End       Size       Free   Next Writable\n");
		printf("--------------------------------------------------------------------------------------------------\n");
		if (dev->media.tracks)
			for (i = 0; i < dev->media.tracks; i++) {
				printf(IMEDIA "Track# %2d       %2d      %2d      %2d   %8d   %8d   %8d   %8d   %8d\n",
				       dev->media.track[i].n, dev->media.track[i].session, dev->media.track[i].track_mode,
				       dev->media.track[i].data_mode, dev->media.track[i].start, dev->media.track[i].last,
				       dev->media.track[i].size, dev->media.track[i].free, dev->media.track[i].next_writable);
			}
		printf(
		    "--------------------------------------------------------------------------------------------------\n\n");
		goto end;
...
//		printf("setting signal handlers...\n");
		...
		if (!strcmp(test, "wt")) {
			...
			if (!scanner->check_test(CHK_ERRC)) { show_available_errc_data(scanner); }
		}
