...
int qscanner::run_wr_transfer() {
	...
	if (WT_simul) argv = add_arg(argv, &argc, "-dummy");
...
int qscanner::run_wr_transfer() {
	...
	for (lba = lba_sta; !stop_req && !err && lba < lba_end; lba += wsize) {
		...
		if ((ubuff >> 11) < ((uint32_t)wsize)) { msleep(20); }
		...
		if (!(lba % bsize) || lba + wsize == lba_end || stat_req || stop_req || !wsize) {
			...
			*/
			...
			printf("lba: %7d    speed: %6.2f X  %6d kB/s, written: %4dMB/%4dMB, Ubuf: %3d%%\r", lba, spdX, spdKB,
			       (lba - lba_sta) >> 9, (lba_end - lba_sta) >> 9, ubufp);
			clock_gettime(CLOCK_MONOTONIC, &blks);
			stat_req = 0;
#ifdef USE_FFLUSH
			fflush(stdout);
...
int qscanner::run_cd_errc() {
	...
	for (; (!stop_req) && lba < lba_end;) {
		...
		printf("cur : %6d | %6.2f X %5d kB/s | %5ld | %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\r", lba, spdX, spdKB,
		       err.bler, err.e11, err.e21, err.e31, err.e12, err.e22, err.e32, err.uncr);
		err_tot += err;
		err_max.EMAX(err);
#ifdef USE_FFLUSH
		fflush(stdout);
	...
	printf("tot : %5ld | %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\n", err_tot.bler, err_tot.e11, err_tot.e21, err_tot.e31,
	       err_tot.e21, err_tot.e22, err_tot.e32, err_tot.uncr);
	printf("max : %5ld | %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\n", err_max.bler, err_max.e11, err_max.e21, err_max.e31,
	       err_max.e21, err_max.e22, err_max.e32, err_max.uncr);
	printf("avg : %5.2f | %5.2f %5.2f %5.2f | %5.2f %5.2f %5.2f | %5.2f\n", err_tot.bler / (float)lba,
	       err_tot.e11 / (float)lba, err_tot.e21 / (float)lba, err_tot.e31 / (float)lba, err_tot.e21 / (float)lba,
	       err_tot.e22 / (float)lba, err_tot.e32 / (float)lba, err_tot.uncr / (float)lba);
#ifdef USE_FFLUSH
	fflush(stdout);
#endif
	return 0;
...
int qscanner::run_dvd_errc() {
	...
	for (; (!stop_req) && lba < lba_end;) {
		...
		if ((lba - lbas) >= 0x80) {
			...
			printf("cur : %7d | %6.2f X %5d kB/s | %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\r", lba, spdX, spdKB, err.pie,
			       pi8, err.pif, err.poe, po8, err.pof, err.uncr);
			pi8 = 0;
			po8 = 0;
			lbas = lba;
		} else {
			printf("cur : %7d | %6.2f X %5d kB/s | %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\r", lba, spdX, spdKB, err.pie,
			       (long)-1, err.pif, err.poe, (long)-1, err.pof, err.uncr);
		}
#ifdef USE_FFLUSH
		fflush(stdout);
#endif
	...
	printf("tot : %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\n", err_tot.pie, err_tot.pie, err_tot.pif, err_tot.poe,
	       err_tot.poe, err_tot.pof, err_tot.uncr);
	printf("max : %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\n", err_max.pie, pi8_max, err_max.pif, err_max.poe, po8_max,
	       err_max.pof, err_max.uncr);
	...
	printf("avg : %5.2f %5.2f %5.2f | %5.2f %5.2f %5.2f | %5.2f\n", err_tot.pie / (float)(lba >> 4),
	       err_tot.pie / (float)(lba >> 7), err_tot.pif / (float)(lba >> 4), err_tot.poe / (float)(lba >> 4),
	       err_tot.poe / (float)(lba >> 7), err_tot.pof / (float)(lba >> 4), err_tot.uncr / (float)(lba >> 4));
#ifdef USE_FFLUSH
	fflush(stdout);
#endif
	return 0;
...
int qscanner::run_dvd_ta() {
	...
	for (int i = 0; i < 3 * dev->media.layers; i++) {
		...
		for (int idx = 0; idx < TA_HIST_SIZE; idx++) { printf("TA %3d  %6d  %6d\r", idx, ta.pit[idx], ta.land[idx]); }
	}
...
int qscanner::run_bd_errc() {
	...
	for (; (!stop_req) && lba < lba_end;) {
		...
		calc_cur_speed(lba - lbao);
		//		if (!(lba & 0x7F)) {
		printf("cur : %7d | %6.2f X %5d kB/s | %5ld %5ld | %5ld\r", lba, spdX, spdKB, err.ldc, err.bis, err.uncr);
#ifdef USE_FFLUSH
		fflush(stdout);
#endif
	}
