...
#endif
	if (set_write_parameters_def(((dev->capabilities & CAP_BURN_FREE)), simul)) { return 1; }
/*
...
int qpxwriter_dvdminus::open_track(uint32_t size) { return (reserve_track(dev, size)); }
...
//  BD-R/RE fonctions
//
int qpxwriter_bd::open_track(uint32_t size) { return (reserve_track(dev, size)); }
