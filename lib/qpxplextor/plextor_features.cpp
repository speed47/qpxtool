...
int plextor_get_speeds(drive_info* drive) {
	...
	if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 0x0A))) {
		if (!drive->silent) sperror("GET_SPEEDS", drive->err);
		return drive->err;
	}
...
int plextor_get_powerec(drive_info* drive) {
	...
	if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
		if (!drive->silent) sperror("GET_POWEREC", drive->err);
		return drive->err;
	}
	...
	if (!drive->silent)
		printf("\tPoweRec %s, Recommended speed: %d kB/s\n", drive->plextor.powerec_state ? "ON" : "OFF",
		       drive->plextor.powerec_spd);
...
int plextor_set_powerec(drive_info* drive) {
	...
	drive->cmd[9] = 0x08;
	if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
		if (!drive->silent) sperror("SET_POWEREC", drive->err);
		return drive->err;
	}
...
void print_speedread_state(drive_info* drive) { printf("\tSpeedRead: %s\n", drive->plextor.spdread ? "on" : "off"); }
...
void print_sss_state(drive_info* drive) { printf("\tSingleSession: %s\n", drive->plextor.sss ? "on" : "off"); }
...
int plextor_get_autostrategy_db(drive_info* drive) //, void* database)
{
	...
	for (j = 0; j < drive->astrategy.dbcnt; j++) {
		...
		printf("S#%02d |%c| DVD%cR [%02X] | %3dX | %12s | %d\n", drive->astrategy.entry[j].number,
		       drive->astrategy.entry[j].enabled ? '*' : ' ', (drive->astrategy.entry[j].type == 0xA1) ? '+' : '-',
		       drive->astrategy.entry[j].type, drive->astrategy.entry[j].speed, drive->astrategy.entry[j].MID,
