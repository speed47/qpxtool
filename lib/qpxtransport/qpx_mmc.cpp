...
int isPlextor(drive_info* drive) {
	if (!strncmp(drive->ven, "PLEXTOR ", 8)) {
		...
		if (!strncmp(drive->dev, "DVDR   PX-708A", 14) || !strncmp(drive->dev, "DVDR   PX-708A2", 15) ||
		    !strncmp(drive->dev, "DVDR   PX-712A", 14) || !strncmp(drive->dev, "DVDR   PX-714A", 14) ||
		    !strncmp(drive->dev, "DVDR   PX-716A ", 15) || !strncmp(drive->dev, "DVDR   PX-716AL", 15) ||
		    !strncmp(drive->dev, "DVDR   PX-755A", 14) || !strncmp(drive->dev, "DVDR   PX-760A", 14))
			return 1;
	}
	return 0;
}
...
int isPlextorLockPresent(drive_info* drive) {
	if (!strncmp(drive->dev, "CD-R   PREMIUM2", 15) || !strncmp(drive->dev, "DVDR   PX-755A", 14) ||
...
int check_write_modes(drive_info* drive) {
	...
	for (int i = 0; wr_modes[i].id; i++) {
		...
		if (!mode_select(drive, 60)) { drive->wr_modes |= wr_modes[i].id; }
	}
...
int get_configuration(drive_info* drive, int feature_number, unsigned int* data_length, int* current,
...
void detect_iface(drive_info* drive) {
	...
	drive->iface_id =
	    (drive->rd_buf[12] << 12) | (drive->rd_buf[13] << 8) | (drive->rd_buf[14] << 4) | drive->rd_buf[15];
...
int test_dma_speed(drive_info* drive, long msecs) {
	...
	if (!drive->silent) printf("** Testing DMA speed...\n");
	st = getmsecs();
...
void detect_capabilities(drive_info* drive) {
	...
	if (drive->mmc > 1) {
		...
		if ((len >= 0x08) && (drive->rd_buf[11] >= 28)) {
			if (drive->rd_buf[16] | drive->rd_buf[17] | drive->rd_buf[18] | drive->rd_buf[19] | drive->rd_buf[20] |
			    drive->rd_buf[21] | drive->rd_buf[22] | drive->rd_buf[23])
				drive->rd_capabilities |= (DEVICE_BD_RE);
			if (drive->rd_buf[24] | drive->rd_buf[25] | drive->rd_buf[26] | drive->rd_buf[27] | drive->rd_buf[28] |
			    drive->rd_buf[29] | drive->rd_buf[30] | drive->rd_buf[31])
				drive->rd_capabilities |= (DEVICE_BD_R);
			if (drive->rd_buf[32] | drive->rd_buf[33] | drive->rd_buf[34] | drive->rd_buf[35] | drive->rd_buf[36] |
			    drive->rd_buf[37] | drive->rd_buf[38] | drive->rd_buf[39])
				drive->rd_capabilities |= (DEVICE_BD_ROM);
		}
...
int get_track_list(drive_info* drive) {
	...
	if (read_track_info(drive, &drive->media.track[0], 1)) {
		...
		for (i = 0; i < drive->media.tracks; i++) read_track_info(drive, &drive->media.track[i], i + 1);
	}
	...
	if (drive->media.type & DISC_CD) {
		...
		if (drive->media.type & (DISC_DVDmRWR | DISC_DVDmRWS | DISC_DVDmRWDL)) {
			...
			while (!drive->media.capacity_total) {
				if (!(drive->rd_buf[doffs + 4] >> 2)) drive->media.capacity_total = ntoh32(drive->rd_buf + doffs);
			}
...
int read_disc_information(drive_info* drive) {
	...
	if (!drive->silent) {
		...
		printf("   Last session  lead-in  start: %d:%02d.%02d\n", (drive->rd_buf[16] << 8) | drive->rd_buf[17],
		       drive->rd_buf[18], drive->rd_buf[19]);
		...
		printf("   Last possible lead-out start: %d:%02d.%02d (sector 0x%08X)\n",
		       (drive->rd_buf[20] << 8) | drive->rd_buf[21], drive->rd_buf[22], drive->rd_buf[23],
		       drive->media.last_lead_out);
	}
	/*
...
int read_mediaid_bd(drive_info* drive) {
	...
	if ((drive->media.type & DISC_BD_ROM) && strncmp((const char*)&drive->media.MID_raw[4 + 8], "BDR", 3) == 0) {
		if (!drive->silent) printf("BD DI info indicates type BD-R\n");
...
int read_mediaid_dvd(drive_info* drive) {
	...
	if (drive->media.type & DISC_DVDminus) {
		...
		if (read_mediaid_dvdplus(drive)) return read_mediaid_dvdminus(drive);
#endif
	}
	return 0;
}
...
int read_mediaid_dvdram(drive_info* drive) {
	...
	for (i = 0; i < 12; i++)
		if (drive->media.MID[i] == 0) drive->media.MID[i] = 0x20;
...
int read_mediaid_cd(drive_info* drive) {
	...
	while (mi[idx].lin.m && nf)
		if (lin.m == mi[idx].lin.m && lin.s == mi[idx].lin.s && lin.f == mi[idx].lin.f)
			nf = 0;
		else
			idx++;
...
int determine_disc_type(drive_info* drive) {
	...
	if (drive->mmc > 1) {
		...
		if (drive->media.type & DISC_CD) {
			...
			if (drive->media.type & DISC_DVDminus) read_writer_info(drive);
...
int get_performance(drive_info* drive, bool rw, uint8_t type) {
	...
	if (type == 0x03)
		for (j = 0; j < speed_tbl_size; j++) drive->parms.wr_speed_tbl_kb[j] = -1;
	...
	for (j = 0; j < std::min(descn, (type == 0x03) ? speed_tbl_size : descn); j++) {
		...
		if (!type) {
			...
			drive->parms.max_write_speed_kb =
			    std::max(drive->parms.max_write_speed_kb, drive->parms.wr_speed_tbl_kb[j]);
		}
	}
	return 0;
}
	...
	if (drive->media.type & (DISC_DVD | DISC_BD)) {
		...
		if (!set_rw_speeds(drive)) {
			...
			if ((drive->capabilities & CAP_REAL_TIME_STREAMING)
				...
				if (drive->perf.spd_e > 1) drive->parms.max_read_speed_kb = drive->perf.spd_e;
			...
			if (!drive->silent) {
				...
				printf("Max spd: %d X, %d kB/s\n", drive->parms.max_read_speed_dvd, drive->parms.max_read_speed_kb);
			}
...
int get_write_speed_tbl(drive_info* drive) {
	...
	if (drive->capabilities & CAP_REAL_TIME_STREAMING) {
		...
		for (i = 0; i < speed_tbl_size; i++) drive->parms.wr_speed_tbl_kb[i] = -1;
		//		printf("== Write speeds: %d\n",spdcnt);
		for (i = 0; (i < spdcnt) && (i < speed_tbl_size); i++) {
			drive->parms.wr_speed_tbl_kb[i] = ntoh16(drive->rd_buf + offs + 32 + i * 4 + 2);
			drive->parms.max_write_speed_kb =
			    std::max(drive->parms.max_write_speed_kb, drive->parms.wr_speed_tbl_kb[i]);
...
*/
	if (!drive->get_performance_fail && isPlextor(drive)) { drive->get_performance_fail = 1; }
	...
	if (!drive->get_performance_fail
		...
		if (!get_performance(drive, 0, 0) && !get_performance(drive, 1, 0)) return 0;
...
int read_cd(drive_info* drive, unsigned char* data, uint32_t lba, int sector_count, unsigned char flags,
            unsigned char FUA) {
...
int get_drive_serial_number(drive_info* drive) {
	...
	if (data_length > 8)
		strncpy(drive->serial, (char*)drive->rd_buf + 12, 32);
	else
		drive->serial[0] = 0;
	return 0;
}
...
int detect_mm_capabilities(drive_info* drive) {
	...
	switch (drive->mmc) {
		...
		case 1:
			...
			if (drive->mmc < 3) drive->parms.write_speed_kb = ntoh16u(drive->rd_buf + offs + 20);
			break;
	}
	if (!drive->silent)
		printf("Max speeds:\tR@%dKBps / W@%dKBps\nCurrent speeds:\tR@%dKBps / W@%dKBps\n",
		       drive->parms.max_read_speed_kb, drive->parms.max_write_speed_kb, drive->parms.read_speed_kb,
		       drive->parms.write_speed_kb);

	if (isPlextor(drive) && strncmp(drive->dev, "CD-R", 4)) drive->capabilities |= CAP_TEST_WRITE_DVD_PLUS;
	return 0;
}
