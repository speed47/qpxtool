...
#define CREAM_ON_ERRNO(s) \
	do { CREAM_ON_ERRNO_NAKED(s) } while (0)
...
#define ERRCODE(s) \
	((s)[0] == 0x70 || (s)[0] == 0x71 ? ERRCODE_FIXED(s) : ((s)[0] == 0x72 || (s)[0] == 0x73 ? ERRCODE_DESCR(s) : 0))
...
typedef enum { NONE = CAM_DIR_NONE, READ = CAM_DIR_IN, WRITE = CAM_DIR_OUT } Direction;
...
typedef enum { NONE = SCSI_IOCTL_DATA_UNSPECIFIED, READ = SCSI_IOCTL_DATA_IN, WRITE = SCSI_IOCTL_DATA_OUT } Direction;
