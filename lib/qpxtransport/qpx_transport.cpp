...
int Scsi_Command::associate(const char* file, const struct stat* ref = NULL) {
	 ...
	 */
	if ((fd = open(file, O_RDWR | O_NONBLOCK)) < 0 && (fd = open(file, O_RDONLY | O_NONBLOCK)) < 0) return 0;
...
int Scsi_Command::associate(const char* file, const struct stat* ref) {
	...
	if (ref && fd < 0 && errno == EPERM) {
		...
		if (!S_ISCHR(ref->st_mode) || ref->st_rdev != sb.st_rdev) return (errno = ENXIO, 0);
		fd = open(file, O_RDWR);
	}
	...
	if (ref && (!S_ISCHR(ref->st_mode) || ref->st_rdev != sb.st_rdev)) return (errno = ENXIO, 0);
...
int Scsi_Command::transport(Direction dir, void* buf, size_t sz) {
	...
	if ((ret = cam_send_ccb(cam, &ccb)) < 0) return -1;

	if ((ccb.ccb_h.status & CAM_STATUS_MASK) == CAM_REQ_CMP) return 0;

	/* cmd length fix */
	//	printf("CMD: (%2d)  %02x\n", ccb.csio.cdb_len, ccb.csio.cdb_io.cdb_bytes[0]);
	ccb.csio.cdb_len = (ccb.csio.cdb_len < COMMAND_SIZE(ccb.csio.cdb_io.cdb_bytes[0])) ?
	...
	if ((ccb.csio.scsi_status == SCSI_STATUS_CHECK_COND) && !(ccb.ccb_h.status & CAM_AUTOSNS_VALID)) {
		u_int8_t _sense[18];
		...
		if (ret < 0) return -1;
		if ((ccb.ccb_h.status & CAM_STATUS_MASK) != CAM_REQ_CMP) return errno = EIO, -1;
...
Scsi_Command::~Scsi_Command() {
	...
	if (fd != INVALID_HANDLE_VALUE && autoclose) {
		if (autoclose > 1) DeviceIoControl(fd, FSCTL_UNLOCK_VOLUME, NULL, 0, NULL, 0, &junk, NULL);
...
int Scsi_Command::associate(const char* file, const struct stat* ref) {
	...
	if (GetDriveType(dev) != DRIVE_CDROM) return errno = EINVAL, 0;
	sprintf(dev, "\\\\.\\%.*s", (int)sizeof(dev) - 5, file);
	fd =
	    CreateFile(dev, GENERIC_WRITE | GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	if (fd != INVALID_HANDLE_VALUE) filename = strdup(dev);
...
int Scsi_Command::transport(Direction dir, void* buf, size_t sz) {
	...
	if (DeviceIoControl(fd, IOCTL_SCSI_PASS_THROUGH_DIRECT, &p, sizeof(p.spt), &p, sizeof(p), &bytes, FALSE) == 0)
		return -1;
...
static int iokit_err(IOReturn ioret, SCSITaskStatus stat, const unsigned char* sense) {
	int ret = -1;
...
Scsi_Command::~Scsi_Command() {
	if (autoclose) {
		if (taskif) (*taskif)->ReleaseExclusiveAccess(taskif), (*taskif)->Release(taskif), taskif = NULL;
...
int Scsi_Command::associate(const char* file, const struct stat* ref) {
	...
	if (!(S_ISBLK(sb.st_mode) || S_ISCHR(sb.st_mode))) return !(errno = ENOTBLK);

	if ((match = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, &kCFTypeDictionaryKeyCallBacks,
	                                       &kCFTypeDictionaryValueCallBacks)) == NULL)
		return !(errno = ENOMEM);
	if ((bsddev = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, &kCFTypeDictionaryKeyCallBacks,
	                                        &kCFTypeDictionaryValueCallBacks)) == NULL)
	...
	if ((scsiob = IOServiceGetMatchingService(kIOMasterPortDefault, match)) == IO_OBJECT_NULL) return !(errno = ENXIO);

	// traverse up to "SCSITaskAuthoringDevice"
	kern_return_t kret;
	while ((kret = IORegistryEntryGetParentEntry(scsiob, kIOServicePlane, &parent)) == kIOReturnSuccess) {
		CFStringRef uclient;
		const char* s;
		int cmp;
		...
		scsiob = parent;
		uclient = (CFStringRef)IORegistryEntryCreateCFProperty(scsiob, CFSTR(kIOPropertySCSITaskDeviceCategory),
	...
	SInt32 score = 0;
	if (IOCreatePlugInInterfaceForService(scsiob, kIOMMCDeviceUserClientTypeID, kIOCFPlugInInterfaceID, &plugin,
	                                      &score) != kIOReturnSuccess) {
		IOObjectRelease(scsiob);
		return !(errno = ENXIO);
	}
	if ((*plugin)->QueryInterface(plugin, CFUUIDGetUUIDBytes(kIOMMCDeviceInterfaceID), (void**)&mmcdif) != S_OK) {
...
int Scsi_Command::umount(int f) {
	...
	if (f >= 0) {
		...
		sprintf(bsdname, "/dev/%.*s", (int)(sizeof(bsdname) - 6), CFStringGetCStringPtr(devname, 0));
		CFRelease(devname);
	...
	for (i = 0; i < n; i++, p++) {
		if (stat(p->f_mntfromname, &sb) == 0 && S_ISBLK(sb.st_mode) && sb.st_rdev == ref)
...
#else
		{
			...
			if (pid == 0) // if diskutil will be proven broken,
				...
				// attack vector...
				setuid(getuid()), execl("/usr/sbin/diskutil", "diskutil", "unmount", p->f_mntonname, (void*)NULL),
				    exit(errno);
			while (1) {
				rpid = waitpid(pid, &rval, 0);
				if (rpid == (pid_t)-1) {
			...
			if (ret == 0) {
				...
				if (statfs(p->f_mntonname, &fs) == 0 && !strcmp(fs.f_mntfromname, p->f_mntfromname))
					return unmount(p->f_mntonname, 0);
			}
			return ret;
		}
