...
static void css_CryptKey(int i_key_type, int i_variant, unsigned char const* p_challenge, unsigned char* p_key);
static void css_DecryptKey(unsigned char invert, unsigned char const* p_key, unsigned char const* p_crypted,
...
static int css_RecoverTitleKey(int i_start, unsigned char const* p_crypted, unsigned char const* p_decrypted,
...
int seek_dvd(drive_info* drive, uint32_t lba, int flags) {
	...
	if (((flags & DVDCSS_SEEK_MPEG) && (drive->media.dvdcss.method != DVDCSS_METHOD_TITLE)) ||
	    (flags & DVDCSS_SEEK_KEY)) {
		/* check the title key */
		if (css_title(drive, lba)) { return -1; }
	}
...
//                                          int i_flags )
{
	...
	if (i_ret <= 0 || drive->media.dvdcss.protection != 0x01 || !(flags & DVDCSS_READ_DECRYPT)) { return i_ret; }
...
int css_report_challenge(drive_info* drive) {
	...
	for (int i = 0; i < (2 * DVD_KEY_SIZE); i++) { drive->media.dvdcss.CK[i] = drive->rd_buf[13 - i]; }
	printf("Report Challenge: %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\n", drive->media.dvdcss.CK[0],
	       drive->media.dvdcss.CK[1], drive->media.dvdcss.CK[2], drive->media.dvdcss.CK[3], drive->media.dvdcss.CK[4],
	       drive->media.dvdcss.CK[5], drive->media.dvdcss.CK[6], drive->media.dvdcss.CK[7], drive->media.dvdcss.CK[8],
	       drive->media.dvdcss.CK[9]);
	return 0;
}
...
int css_send_challenge(drive_info* drive) {
	...
	for (int i = 0; i < (2 * DVD_KEY_SIZE); i++) { drive->rd_buf[13 - i] = drive->media.dvdcss.CK[i]; }
	drive->rd_buf[14] = 0;
	drive->rd_buf[15] = 0;
...
int css_report_key1(drive_info* drive) {
	...
	for (int i = 0; i < DVD_KEY_SIZE; i++) { drive->media.dvdcss.K1[i] = drive->rd_buf[8 - i]; }
	printf("Report KEY1: %02X:%02X:%02X:%02X:%02X\n", drive->media.dvdcss.K1[0], drive->media.dvdcss.K1[1],
	       drive->media.dvdcss.K1[2], drive->media.dvdcss.K1[3], drive->media.dvdcss.K1[4]);
	return 0;
}
...
int css_send_key2(drive_info* drive) {
	...
	printf("Send KEY2: %02X:%02X:%02X:%02X:%02X\n", drive->media.dvdcss.K2[0], drive->media.dvdcss.K2[1],
	       drive->media.dvdcss.K2[2], drive->media.dvdcss.K2[3], drive->media.dvdcss.K2[4]);
	...
	drive->rd_buf[2] = 0;
	drive->rd_buf[3] = 0;
	for (int i = 0; i < DVD_KEY_SIZE; i++) { drive->rd_buf[8 - i] = drive->media.dvdcss.K2[i]; }
	drive->rd_buf[9] = 0;
	drive->rd_buf[10] = 0;
	drive->rd_buf[11] = 0;
...
int css_disckey(drive_info* drive) {
	...
	if (css_get_bus_key(drive)) { return -1; }
	...
	for (i = 0; i < DVD_DISCKEY_SIZE; i++) { p_buffer[i] ^= drive->media.dvdcss.BK[4 - (i % DVD_KEY_SIZE)]; }
...
int css_unscramble(dvd_key_t p_key, unsigned char* p_sec) {
	...
	if (!(p_sec[0x14] & 0x30)) { return 0; }

	i_t1 = (p_key[0] ^ p_sec[0x54]) | 0x100;
	i_t2 = p_key[1] ^ p_sec[0x55];
	i_t3 = (p_key[2] | (p_key[3] << 8) | (p_key[4] << 16)) ^ (p_sec[0x56] | (p_sec[0x57] << 8) | (p_sec[0x58] << 16));
	i_t4 = i_t3 & 7;
	i_t3 = i_t3 * 2 + 8 - i_t4;
	p_sec += 0x80;
	i_t5 = 0;
...
int css_get_bus_key(drive_info* drive) {
	...
	for (i = 0; i < 10; ++i) { drive->media.dvdcss.CK[i] = i; }
...
static void css_CryptKey(int i_key_type, int i_variant, unsigned char const* p_challenge, unsigned char* p_key) {
	...
	unsigned char pp_perm_variant[2][32] = {
	    {0x0a, 0x08, 0x0e, 0x0c, 0x0b, 0x09, 0x0f, 0x0d, 0x1a, 0x18, 0x1e, 0x1c, 0x1b, 0x19, 0x1f, 0x1d,
	     0x02, 0x00, 0x06, 0x04, 0x03, 0x01, 0x07, 0x05, 0x12, 0x10, 0x16, 0x14, 0x13, 0x11, 0x17, 0x15},
	    {0x12, 0x1a, 0x16, 0x1e, 0x02, 0x0a, 0x06, 0x0e, 0x10, 0x18, 0x14, 0x1c, 0x00, 0x08, 0x04, 0x0c,
	     0x13, 0x1b, 0x17, 0x1f, 0x03, 0x0b, 0x07, 0x0f, 0x11, 0x19, 0x15, 0x1d, 0x01, 0x09, 0x05, 0x0d}};

	unsigned char p_variants[32] = {0xB7, 0x74, 0x85, 0xD0, 0xCC, 0xDB, 0xCA, 0x73, 0x03, 0xFE, 0x31,
	...
	for (i = 9; i >= 0; --i) p_scratch[i] = p_challenge[pp_perm_challenge[i_key_type][i]];
     ...
     */
	for (i = 5; --i >= 0;) { p_tmp1[i] = p_scratch[5 + i] ^ p_secret[i] ^ p_crypt_tab2[i]; }
	...
	do {
		for (i_bit = 0, i_val = 0; i_bit < 8; ++i_bit) {
			i_lfsr0_o = ((i_lfsr0 >> 24) ^ (i_lfsr0 >> 21) ^ (i_lfsr0 >> 20) ^ (i_lfsr0 >> 12)) & 1;
 ...
 *****************************************************************************/
static int css_DecryptDiscKey(drive_info* drive, unsigned char const* p_struct_disckey, dvd_key_t p_disc_key) {
	dvd_key_t p_verify;
	unsigned int i, n = 0;
	...
	for (n = 0; n < sizeof(player_keys) / sizeof(dvd_key_t); n++) {
		...
		for (i = 1; i < 409; i++) {
			...
			if (memcmp(p_disc_key, p_verify, DVD_KEY_SIZE) == 0) { return 0; }
		}
	}
		...
		for (nStepB = 0; nStepB < 256; nStepB++) {
			...
			for (nTry = 0; nTry < nPossibleK1; nTry++) {
				...
				if ((B[1] ^ p_css_tab1[B[2]] ^ k[2]) == C[2]) {
					if (!css_investigate(&p_disc_key[0], &C[0])) { goto end; }
				}
...
static int css_RecoverTitleKey(int i_start, unsigned char const* p_crypted, unsigned char const* p_decrypted,
	...
	for (i = 0; i < 10; i++) { p_buffer[i] = p_css_tab1[p_crypted[i]] ^ p_decrypted[i]; }
	...
	for (i_try = i_start; i_try < 0x10000; i_try++) {
		...
		for (i = 0; i < 4; i++) {
			...
			/* deduce i_t6 & i_t5 */
			i_t6 = p_buffer[i];
			if (i_t5) { i_t6 = (i_t6 + 0xff) & 0x0ff; }
			if (i_t6 < i_t4) { i_t6 += 0x100; }
			i_t6 -= i_t4;
			i_t5 += i_t6 + i_t4;
			i_t6 = p_css_tab4[i_t6];
 ...
 *****************************************************************************/
static int css_CrackTitleKey(drive_info* drive, int i_pos, int i_len, dvd_key_t p_titlekey) {
	...
	do {
		...
		if (i_ret) { printf("CSS: seek failed\n"); }
		...
		if (p_buf[0x0d] & 0x07) printf("CSS: stuffing in pack header\n");

		/* PES_scrambling_control does not exist in a system_header,
         * a padding_stream or a private_stream2 (and others?). */
		if (p_buf[0x14] & 0x30 && !(p_buf[0x11] == 0xbb || p_buf[0x11] == 0xbe || p_buf[0x11] == 0xbf)) {
			i_encrypted++;

			if (css_AttackPattern(p_buf, i_reads, p_titlekey) > 0) { b_stop_scanning = 1; }
#if 0
		...
		if (!(i_reads & 0xfff)) { printf("CSS: at block %i, still cracking...\n", i_pos); }
	...
	if (!b_stop_scanning) { printf("CSS: end of title reached\n"); }
...
static int css_AttackPattern(unsigned char const p_sec[DVDCSS_BLOCK_SIZE], int i_pos, unsigned char* p_key) {
	...
	for (i = 2; i < 0x30; i++) {
		...
		for (j = i + 1; j < 0x80 && (p_sec[0x7F - (j % i)] == p_sec[0x7F - j]); j++) {
	...
	if ((i_best_plen > 3) && (i_best_plen / i_best_p >= 2)) {
		...
		res = css_RecoverTitleKey(0, &p_sec[0x80], &p_sec[0x80 - (i_best_plen / i_best_p) * i_best_p],
...
int css_title(drive_info* drive, uint32_t lba) {
	...
	if (!(drive->media.dvdcss.protection == 0x01)) { return 0; }

	/* Check if we've already cracked this key */
	p_title = drive->media.dvdcss.p_titles;
	while (p_title != NULL && p_title->p_next != NULL && p_title->p_next->i_startlb <= lba) {
...
int css_titlekey(drive_info* drive, uint32_t lba, dvd_key_t p_title_key) {
	...
	if (drive->media.dvdcss.method == DVDCSS_METHOD_KEY || drive->media.dvdcss.method == DVDCSS_METHOD_DISC) {
		...
		if (!(i_ret < 0)) {
			...
			for (i = 0; i < DVD_KEY_SIZE; i++) { p_key[i] ^= drive->media.dvdcss.BK[4 - (i % DVD_KEY_SIZE)]; }
